...
BSTree bstree_k_esimo_menor_helper(BSTree raiz, int *contador, int k) {
    if (raiz != NULL && *contador < k) {
        BSTree izquierdo = bstree_k_esimo_menor_helper(raiz->izq, contador, k);
        ...
        BSTree derecho = bstree_k_esimo_menor_helper(raiz->der, contador, k);
        
        // Si el elemento se encuentra en la rama izquierda, devuelve el resultado de la rama izquierda
...
// Función principal para encontrar el k-ésimo menor elemento
BSTree bstree_k_esimo_menor(BSTree arbol, int *contador, int k) {
    return bstree_k_esimo_menor_helper(arbol, contador, k);
}
